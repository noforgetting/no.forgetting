...
export function initAuth() {
  ...
  window.addEventListener("noforgetting:sync-status", (event) => { const { state, message, user } = event.detail; status.textContent = message; title.textContent = user ? "Your reminders are synced" : "Sync reminders"; description.textContent = user ? "All your reminders are synced to signed in devices" : "Sign in to access your reminders on all devices"; open.textContent = user ? "Sign out" : "Sign in to sync"; panel.classList.toggle("is-error", state === "error"); });
