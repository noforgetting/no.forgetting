...
export function saveData(appData, { sync = true } = {}) {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(normalizeStudyData(appData)));
  window.dispatchEvent(new CustomEvent("noforgetting:data-changed", { detail: { sync } }));
  if (sync) import("./sync.js").then(({ queueSync }) => queueSync());
}
