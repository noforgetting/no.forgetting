...
function renderTests() {
  ...
  elements.testList.innerHTML = sortedTests.map((test) => {
    ...
    const badgeClass = days < 0 || days === 1 ? "danger" : days <= 3 ? "warning" : "success";
...
function renderPersonalReminders() {
  ...
  elements.personalReminderList.innerHTML = personalReminders.map((reminder) => {
    ...
    return `
      <div class="list-item">
        <div>
          <p class="item-title">${escapeHTML(reminder.title)} <span class="badge badge--type">${escapeHTML(reminder.type)}</span></p>
